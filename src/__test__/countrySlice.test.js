...
import countriesSlice, { fetchCountryData } from '../redux/countries/countriesSlice';
...
describe('Homepage', () => {
  it('should render data fetched from the API', async () => {
    ...
    expect(getByText('Ecuador')).toBeInTheDocument();
    // Add more assertions if needed
  });
});
describe('dataSlice reducers', () => {
  it('should set isLoading to true when fetchCountryData is pending', () => {
    const initialState = {
      isLoading: 'idle',
    };
    const nextState = countriesSlice(initialState, fetchCountryData.pending());
    expect(nextState.loading).toBe('pending');
  });

  it('should set isLoading to false and update the data when fetchCountryData is fulfilled', () => {
    const initialState = {
      isLoading: 'idle',
      countries: [],
      selectedCountry: null,
    ...
    const nextState = countriesSlice(initialState, fetchCountryData.fulfilled(mockPayload));
    expect(nextState.loading).toBe('fulfilled');
    expect(nextState.countries).toHaveLength(2);
  });

  it('should set isLoading to false and update the error when fetchCountryData is rejected', () => {
    const initialState = {
      countries: [],
      selectedCountry: null,
      error: null,
    ...
    const nextState = countriesSlice(initialState, fetchCountryData.rejected(mockErrorPayload));
